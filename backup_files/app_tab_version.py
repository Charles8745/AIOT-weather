...
def get_cached_week_forecast(city):
    ...
    try:
        ...
        print(f"Error in get_cached_week_forecast: {e}")
        return []
...
with tab1:
    if week_data:
        ...
        # 一週預報卡片
        if len(week_data) > 0:
            cols = st.columns(min(7, len(week_data)))
            ...
            for idx, day_data in enumerate(week_data[:7]):
                with cols[idx]:
                    ...
                    <div class="forecast-day">
                        ...
                        <div class="day-name">{date_str}</div>
                        <div class="icon">{icon}</div>
                        <div class="temp-high">{day_data.get("max_temp", "--"):.0f}°</div>
                        <div class="temp-low">{day_data.get("min_temp", "--"):.0f}°</div>
                        <div class="temp-low">💧 {day_data.get("pop", 0):.0f}%</div>
                    </div>
                    ''', unsafe_allow_html=True)
    else:
        st.info('📊 暫無一週預報資料')
